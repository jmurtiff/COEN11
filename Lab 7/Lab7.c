...
/* user presses 1, then the program first reads a text file       */
/* containing a header for name and number of guests in the user's*/
/* party as well as the actual names and number of guests in the  */
/* party. If no information exists, then a file will be created   */
/* instead. Once the file has been read and the name and number of*/
/* guests has been read, this information is then inserted into an*/
/* array of linked lists where one node contains the user's name, */
/* number of guests associated with the user's party, and a       */
/* pointer to the next node in the list. Depending on the number  */
/* of guests the user enters, a node will be created in a specific*/
/* linked list depending on the range. Linked lists will contain  */
/* parties from 1-2, 3-4, 5-6, and parties greater than 7. Each   */
/* new entry will go into one of ranges listed above. If the user */
/* presses 2, then the program asks for the number of guests in   */
/* user's party and removes the specific node containing less than*/
...
/* the program asks for the user's name and asks for a new number */
/* of guests. If the user enters a new number of guests that is   */
/* lower or higher than the original reservation range, than it   */
/* changes that node to be at the end of the respective linked    */
/* list. If the user presses 5, the program writes the information*/
/* stored in each node in eahc linked list to a file and then     */
/* quits the program.						  */
...
{
	//Create a file pointer that points to the file we are reading and writing in.
	FILE *fp;
	...
	//Create a variable that stores the name of the file we are reading and writing from.
	...
	else
	{
		...
		//Tries to open the file for reading the names and group sizes of every party
		fp = fopen(argv[1], "r");
		
			//If the name is wrong or the file does not exist, crete a new file and print the header inside. At the end. make sure to set the file to reading mode.	
		if (fp == NULL)
		{
			printf("File unable to be found, creating new file now\n\n");
			fp = fopen(argv[1], "w");
			fprintf(fp,"%s\n", header);
			fclose(fp);
			fp = fopen(argv[1], "r");	
	...
	printf("Enter 5 to write data to a file, deallocate memory, and quit the program\n");
...
{
	//If a file is being read and there are parties to be read, execute this insert function instead of the normal insert function.
	if(name != '\0')
	{
		printf("Hit here\n");		
...
void quitProgram(char* nameOfFile)
{
	//Creates a file pointer that will point to the file we are trying to write to.
	FILE *fp;	

	//Opens the file in writing mode so we can write in the names and guest numbers of all the parties.
	fp = fopen(nameOfFile, "w");

	//Creates a variable to hold the header string for the file.
	char header [] = "Name\t\tGroup Size\n--------------------------\n";

	//Write the header to the file.
	fprintf(fp,"%s", header);

	//A for loop traverses each linked list and writes the party information into the file.
	int i;	
	for(i=0; i<4; i++)
	{
		struct waitingList *ptr = heads[i];
