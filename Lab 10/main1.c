...
/* found, a file is created instead with a header included. In    */ 
/* addition, a thread runs continuously throughout the program    */ 
/* that autosaves all the data within the linked list into an     */
/* text file every 5 seconds (unless a function is being run that */
/* changes the list in any way. If the user presses 1, then the   */
/* program asks for the user's name and number of guests and puts */
/* this information into an array of linked lists, where one node */
/* contains the user's name, number of guests associated with the */
...
/* specific node containing less than or equal to the number of   */
/* guests entered. The program searches through the array of      */
/*linked lists to find the oldest entry matching this criteria    */
/* and deletes the node. If the user presses 3, then the program  */
/* prints out every node in the linked list in the array of linked*/
/* lists. If the user presses 4, then the program asks for the    */
/* program asks for the user's name and asks for a new number of  */
/* guests. If the user enters a new number of guests that is lower*/
/* or higher than the original reservation range, than it changes */
/* that node to be at the end of the respective linked list. If   */
/* the user presses 5, then the program erases all the nodes from */
/* the array of linked lists and reads information from the binary*/
/* file into the array of linked list. The nodes in the array of  */
/* linked lists are printed out. If the user presses 6, then the  */
/* program reverses the order of each linked list in the array of */
/* linked lists. If the user presses 5, then the program writes   */
/* all the data from the array of linked lists into a text and    */
/* binary file (header included with the text file) and then quits*/
/* the program.                                                   */		
...
//Declare a mutex variable to lock the thread.
pthread_mutex_t mtx;

//Declare a global thread that will be used in the autosave function.
pthread_t thr;

int main(int argc, char*argv[])
{
	//Intialize  a thread for the autosave function.
	pthread_create(&thr, NULL, autoSaveFile, NULL);

	//Initilaize the mutex variable to lock and unlock functions.
	pthread_mutex_init(&mtx,NULL);

	//Create a file pointer that points to the text and binary file we are reading and writing in.
	FILE *fp1; //Text file
