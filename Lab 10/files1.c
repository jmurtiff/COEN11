...
void quitProgram(char* nameOfTextFile, char* nameOfBinaryFile)
{
	...
	//Show a sucessfull deallocation of nodes and cancel the thread when the program quits.
	pthread_cancel(thr);
	printf("All nodes have been freed.\n");
}
...
void* autoSaveFile(void* arg)
{
	//Infinite loop so the thread continues to save itself every 5 seconds
	while(1)
	{
		//Lock the thread while the autosave process is occuring
		pthread_mutex_lock(&mtx);

		//Creates a file pointer that will point to the autosave text file.
		FILE *fp1;	

		//Opens the file in writing mode so we can write in the names and guest numbers of all the parties.
		fp1 = fopen("auto-save.txt", "w");
		...
		printf("File Autosaved\n");
		pthread_mutex_unlock(&mtx);
		
		//Stop the thread from running again for 5 seconds.
		sleep(5);
	}
}
